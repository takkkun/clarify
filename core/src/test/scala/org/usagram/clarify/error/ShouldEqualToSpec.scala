...
class ShouldEqualToSpec extends FunSpec {
  import Matchers._

  describe("#message") {
    val error = ShouldEqualTo("a value")

    describe("when Tags#label is Some(string)") {
      val tags = Tags(Indefinite(Some("a label")))

      it("returns a message include the label") {
        error.message(tags) should be("a label should equal to a value")
      }
    }
    ...
    describe("when Tags#label is None") {
      ...
      it("returns a message include the default label") {
        error.message(tags) should be("(no label) should equal to a value")
      }
    }
  }
}
